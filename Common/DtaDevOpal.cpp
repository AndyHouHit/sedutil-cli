...
uint8_t DtaDevOpal::initialSetup(char * password, bool securemode)
{
    ...
    std::shared_ptr<SecureString> newpwd;
	uint8_t lastRC;
	...
	if ((lastRC = setLockingRange(0, OPAL_LOCKINGSTATE::READWRITE, "Admin1", (securemode)?(char*)newpwd->c_str():password)) != 0) {
		LOG(E) << "Initial setup failed - unable to set global locking range RW";
		return lastRC;
	}
	if ((lastRC = setMBRDone(1, "Admin1", (securemode)?(char*)newpwd->c_str():password)) != 0){
